...
- 整形常量 1 890
- 实型常量 1.2 3.14
- 字符常量 '\t' '\n' '\0' '\015'(8进制) '\x7f' '\018'(错误的表示！！) 由单引号引起来的单个字符或转义字符
- 字符串常量 "" "a" "abXYZ" "abc\n\021\010"(a b c \n \021 \0 1 8)
- 标识常量 
##### 宏的用法
``` c++
...
- static 静态型 自动初始化为0值或空值 并且static变量的值有继承性。另外常用来修饰一个变量或者函数(防止当前函数对外扩展，使定义的函数只能在当前文件中运行)
...
```
变量的生命周期和作用范围（内部屏蔽外部，优先考虑最近的变量）
- 全局变量和局部变量
- 局部变量和局部变量
全局变量作用域为变量定义开始到当前程序结束，当其他程序需要调用时，可用说明型extern引用
...
将操作数中的第n位置1 其他位不变 num = num | 1 << n;<br>
将操作数中的第n位置0 其他位不变 num = num & ~(1<<n);<br\>
测试第n位: if(num & (1<<n))
...
### 常用函数
- strlen & sizeof       //strlen计算字符串长度（不包括尾0），sizeof统计数组长度
- strcpy & strncpy      //后者指定复制字符串长度
- strcat & strncat      //追加，同上
...
``` c++
int i = 1;
int *p = &i;        //指针字节长度固定，指针数据类型为指针指向内容的数据类型       指针数据类型为：数据类型 *name
int ** q = &p;
...
### 指针与一维数组<br>
**指针为地址变量，数组名为地址常量。这是最本质的区别，其余情况可相互完全替换**   //常量在过程中不可变<br>

### 行指针与列指针<br>
**每一行看成单独的一维数组，而这一行的值就是当前一维数组的数组名**<br>
两个重要概念：行指针和列指针<br>
行指针：指的是一整行，不指向具体元素<br>
列指针：指的是一行中某个具体元素<br>
可以将列指针理解为行指针的具体元素，行指针理解为列指针的地址<br>
那么两个概念之间的具体转换是：<br>
```
*行指针----列指针
&列指针----行指针
...
字符指针指向字符常量，而指针本身是变量，所以可以直接赋值<br>
字符数组本身是一个常量，不可直接赋值
...
``` c++
int a[2][3] = {{1, 2, 3},{ 4, 5, 6}};
int (*q)[3] = a;        //q指针跨度为三个int长度     q和a本质的区别就是q为变量，a为常量
int *p = *a;        //a类似于数组指针，跨度为三个int         p就是一个int长度的指针
```
...
- free代表着变量p不再拥有原来指向内存空间的引用权限，而原来内存中的内容没有清除
- free后最好马上将指针置NULL
